...
deploy_nginx() {
    ...
    # 这里的 'EOF' 确保 Nginx 内部的 ~* 和 $ 符号不被 Shell 破坏
    cat > "$CONF.tmp" << 'EOF'
server {
...
EOF
    ...
    if nginx -t; then
        ...
        echo -e "${GREEN}部署完成！请通过 https://$D:40889 访问。${NC}"
    else
...
while true; do
    clear
    echo -e "${CYAN}--- NAT Pro Manager V5 (Stable) ---${NC}"
    ...
    case $OPT in
        ...
        3|4) 
            ...
            if [[ ! -f "$SSL_DIR/$D/fullchain.pem" ]]; then
                echo -e "${YELLOW}未检测到证书，请先执行选项 2 申请。${NC}"
            else
